...
// Import all engines
const AgeContextualizer = require('./production code/age_contextualizer');
const ToneDeliveryEngine = require('./production code/tone_delivery_engine');
const LanguageEngine = require('./production code/language_engine');
const NarrativeWeaver = require('./production code/narrative_weaver');
...
class UniversalLessonOrchestrator {
  ...
  async loadLessonDNA(lessonId) {
    // Try to load existing DNA file
    const dnaPath = path.join('production code', `${lessonId}_dna.json`);
    try {
      const data = await fs.readFile(dnaPath, 'utf8');
      return JSON.parse(data);
    } catch (err) {
      // If not found, auto-generate DNA from curriculum and template
      if (err.code === 'ENOENT') {
        const topic = await this.findTopicInCurriculum(lessonId);
        if (!topic) throw new Error(`Topic not found in curriculum for lessonId: ${lessonId}`);
        const templatePath = path.join('production code', 'molecular_biology_dna.json');
        const templateData = await fs.readFile(templatePath, 'utf8');
        const template = JSON.parse(templateData);
        // Fill in topic-specific fields
        template.lesson_id = lessonId;
        template.title = topic.title;
        template.learning_objective = topic.learning_objective;
        template.date = topic.date;
        template.day = topic.day;
        // Save new DNA file
        await fs.writeFile(dnaPath, JSON.stringify(template, null, 2), 'utf8');
        return template;
      } else {
        throw err;
      }
  ...
  async findTopicInCurriculum(lessonId) {
    // Search all curriculum files in 'production code/' for the topic
    const months = [
    ...
    for (const month of months) {
      try {
        const filePath = path.join('production code', `${month}_curriculum.json`);
        const data = await fs.readFile(filePath, 'utf8');
        const json = JSON.parse(data);
        const found = json.days.find(day => (day.title && day.title.toLowerCase().replace(/[^a-z0-9]/g, '_') === lessonId));
        if (found) return found;
      } catch (err) { /* ignore missing months */ }
    }
    return null;
  }
